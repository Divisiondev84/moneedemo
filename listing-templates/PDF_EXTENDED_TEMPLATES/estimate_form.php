...
<div class="medium caps tiny-mar">ESTIMATE</div>

<div class="left half">
    <div class="tiny-mar"><?php echo date("m/d/Y",get_gf_e('date',$_GET['gf_pdf'])); ?></div>
    <div class="tiny-mar"><?php gf_e('estimate_number',$_GET['gf_pdf']); ?></div>
	<?php if(get_gf_e('sales_representative_name',$_GET['gf_pdf'])): ?>
        <div class="tiny-mar">
            SG360° contact: <?php gf_e('sales_representative_name',$_GET['gf_pdf']); ?>
        </div>
        <?php else: ?>
        <div class="tiny-mar">
            SG360° contact: <?php gf_e('sales_representative',$_GET['gf_pdf']); ?> 
        </div>
    <?php endif; ?>
</div>

<div class="right half tiny-mar">
	<div class="tiny-mar"><?php gf_e('customer',$_GET['gf_pdf']); ?></div>
	<?php if(get_gf_e('wpcf-customer_address',$_GET['gf_pdf'])): ?>
		<div class="tiny-mar"><?php gf_e('wpcf-customer_address',$_GET['gf_pdf']); ?></div>
    <?php endif; ?>
	<?php if(get_gf_e('wpcf-customer_city',$_GET['gf_pdf'])): ?>
		<div class="tiny-mar"><?php gf_e('wpcf-customer_city',$_GET['gf_pdf']); ?> <?php gf_e('wpcf-customer_state',$_GET['gf_pdf']); ?>, <?php gf_e('wpcf-customer_zip',$_GET['gf_pdf']); ?></div>
    <?php endif; ?>
</div>
...
<div><span class="medium">project name:</span><br/> <?php gf_e('project_name',$_GET['gf_pdf']); ?></div>

<?php if(get_gf_e('cover_letter',$_GET['gf_pdf'])): ?>
	<div><?php gf_e('cover_letter',$_GET['gf_pdf']); ?></div>
<?php endif; ?>

<div><?php echo nl2br(get_gf_e('wpcf-cover_letter',$_GET['gf_pdf'])); ?></div>

<div><span class="medium">project description:</span><br/> <?php echo nl2br(get_gf_e('simple_project_description',$_GET['gf_pdf'])); ?></div>

<div>
    <span class="medium">medium(s):</span>
    <?php 
...
$task_item[7]['name'] = 'comprehensives:'; 
$task_item[7]['hours'] = 'wpcf-comprehensives_hours'; 
$task_item[7]['fee'] = 'wpcf-comprehensives_fee'; ?> 

<?php foreach($task_item as $item) { ?>
    <?php if(get_gf_e($item['hours'],$_GET['gf_pdf'])): ?>
        <div> 
                <?php 
                echo '<span class="medium">'.$item['name'].'</span> '; ?> <?php gf_e($item['fee'],$_GET['gf_pdf']); 
                $status[] = '';
...
<?php if(get_gf_e('photography_fee',$_GET['gf_pdf']) && get_gf_e('photography_fee',$_GET['gf_pdf']) != '' && get_gf_e('photography_fee',$_GET['gf_pdf']) != '$0'): ?>
<div><span class="medium">photography:</span> <?php gf_e('photography_fee',$_GET['gf_pdf']); ?></div>
<?php endif; ?> 

<?php if(get_gf_e('wpcf-illustration_fee',$_GET['gf_pdf']) && get_gf_e('illustration_fee',$_GET['gf_pdf']) != '' && get_gf_e('illustration_fee',$_GET['gf_pdf']) != '$0'): ?>
...
<?php endif; ?>  
    ...
    <div class="other"><span class="medium">other:</span> <?php echo money_format ('%n',$other_total); ?></div>
    <div class="sub-items">
    <?php $other_count = 0;
        while($other_count < $count) {
			if($array_service[$other_count] == ''){continue;}
            echo '<div class="sub-item"><span class="medium">'.$array_service[$other_count].':</span> '. $array[$other_count].'</div>';
			$other_count++;
        }
    ?>
    </div>
...
<div class="medium half-mar">total: <?php gf_e('total_fee',$_GET['gf_pdf']); ?></div>
